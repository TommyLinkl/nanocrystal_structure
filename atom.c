...
int removeDuplicatedAtoms(atom *atoms, int nAtoms, int nMaxBonds) {
	int i, j, nUniqueAtoms = 0;
	int *uniqueAtomFlag;
	vector diff;
	atom *uniqueAtoms;

	// Dynamically allocate memory
	uniqueAtomFlag = (int *) calloc(nAtoms, sizeof(int));
	...
	// Copy just the unique atoms to the uniqueAtoms structure
	for (i = 0; i < nAtoms; i++) {
		if (uniqueAtomFlag[i]) {
...
void deepCopySingleAtom(atom *newAtoms, int newIndex, atom *origAtoms, int origIndex, int nMaxBonds) {
	...
	newAtoms[newIndex].isMetalAtomFlag = origAtoms[origIndex].isMetalAtomFlag;
