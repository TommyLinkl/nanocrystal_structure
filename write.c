...
void writeAllBondsWithPositions(atom *atoms, param params) {
  ...
  pf = fopen("allNeighborBonds.dat", "w");
  for (i = 0; i < params.nAtoms; i++) {
    fprintf(pf, "%s %d % .5f % .5f % .5f  ", atoms[i].symbol, i, atoms[i].pos.x, 
                                             atoms[i].pos.y, atoms[i].pos.z);
    for (k = 0; k < params.nMaxBonds;  k++) {
      if (atoms[i].neighborPos[k].mag > EPS) {
        fprintf(pf, "%s %d % .5f % .5f % .5f  ", atoms[atoms[i].neighborList[k]].symbol, atoms[i].neighborList[k], 
                                                 atoms[i].neighborPos[k].x, atoms[i].neighborPos[k].y, 
                                                 atoms[i].neighborPos[k].z);
      }  
    }
    fprintf(pf, "\n");
  }
