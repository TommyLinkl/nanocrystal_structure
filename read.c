...
void readInput(param *params) {
	FILE *pf;
	int k, j, i = 0;
	char field[1000], tmp[1000], tmpAtomSymbol[4];
	...
	params->chiralFlag = 0;
	params->doNotCutCdSurfaceFlag = 0;
	params->allCdSurfaceFlag = 0;
	params->nHalfLayersToAdd = 0;
	//params->increaseAttachedNCSep = 0; // default to 0 (false)
	...
	if( access( "input.par", F_OK) != -1 ) {
		...
		while (fscanf(pf, "%s", field) != EOF && i < 16) {
			if (! strcmp(field, "confFile")) {
				...
				for (j = 0; j < params->nHalfLayersToAdd; ) {
					...
					for (k = 1; k < params->newLayer[j].nHalfLayers; k++) {
						...
						strcpy(params->newLayer[j+k].metalAtomSymbol, params->newLayer[j].metalAtomSymbol);
						strcpy(params->newLayer[j+k].nonMetalAtomSymbol, params->newLayer[j].nonMetalAtomSymbol);
					}
					j += k;
				}
				if (j != params->nHalfLayersToAdd) {
					inputError("The number of layers listed after nHalfLayersToAdd is incorrect!");
				}
			}
			...
			// TODO: remove this and replace with above makeChiralNC
			else if (! strcmp(field, "chiralFlag")) fscanf(pf, "%s %d", tmp, &(params->chiralFlag));
			else if (! strcmp(field, "doNotCutCdSurfaceFlag")) fscanf(pf, "%s %d", tmp, &(params->doNotCutCdSurfaceFlag));
			else if (! strcmp(field, "allCdSurfaceFlag")) fscanf(pf, "%s %d", tmp, &(params->allCdSurfaceFlag));
			else {
				...
				printf("chiralFlag = 1 (0 is default)\n");
				printf("doNotCutCdSurfaceFlag = 1 (0 is default)\n");
				printf("allCdSurfaceFlag = 1 (0 is default)\n");
...
void readConf(atom *atoms, param *params) {
  ...
  if (pf) {
    ...
    for (i = 0; i < params->nAtoms; i++) {
      ...
      if (! strcmp(params->confFileUnits, "atomic")) {
      	atoms[i].pos = retScaledVector(atoms[i].pos, AUTOANG); // converted positions from a.u. to Ang
      }
      else {
      	atoms[i].pos.mag = retVectorMagnitude(atoms[i].pos);
      }
    }
    fclose(pf);
  }
...
// Also stores whether or not the atom is a metal (1 = true, 0 = false) 
...
int assignAtomTypes(atom *atoms, int nAtoms) {
    ...
    for (i = 0; i < nAtoms; i++) { 
        ...
        if ( isAtomAMetal(atoms[i].symbol) ) {
        	atoms[i].isMetalAtomFlag = 1;
        }
    }
...
/****************************************************************************/
// Returns 1 (true) if atomSymbol == "Cd" or "Zn" or "In" or "Ga"
// else returns 0 (false)

int isAtomAMetal(char *atomSymbol) {

	if (! strcmp(atomSymbol, "Cd") || ! strcmp(atomSymbol, "Cdz") || 
		! strcmp(atomSymbol, "Zn") || ! strcmp(atomSymbol, "Znz") ||
		! strcmp(atomSymbol, "In") || ! strcmp(atomSymbol, "Ga")) {
		return 1;
	}
	else {
		return 0;
...
double retAtomMass(char *atomSymbol) {
	...
	else if (! strcmp(atomSymbol, "In")) return 114.82;
	else if (! strcmp(atomSymbol, "Ga")) return 69.723;
