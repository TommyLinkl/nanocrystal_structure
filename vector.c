...
vector retAbsoluteValueOfVector(vector vect) {
  vect.x = fabs(vect.x); vect.y = fabs(vect.y); vect.z = fabs(vect.z);
  vect.mag = retVectorMagnitude(vect);
...
vector retXYPlaneRotatedVector(vector vect, double rotationAngle) {
  vector rotatedVector;
  double cosRotAngle = cos(rotationAngle);
  double sinRotAngle = sin(rotationAngle);

  rotatedVector.x = cosRotAngle*vect.x - sinRotAngle*vect.y;
  rotatedVector.y = sinRotAngle*vect.x + cosRotAngle*vect.y;
  rotatedVector.z = vect.z;
  rotatedVector.mag = vect.mag;
